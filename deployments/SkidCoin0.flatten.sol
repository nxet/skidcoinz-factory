...
// File contracts/SkidCoin.sol

// WARNING
// Due to the extensive rebase of the git repository before publishing
// on GitHub, the actual code used to deploy the first version of the token is
// lost with the original commit that held it.
// The following code is a modified version of the file in the original commit
// which does NOT compile to the same bytecode as deployed.
...
contract SkidCoin is ERC20Burnable {
  ...
  /// @notice Amount of token decimals to mint and send to liquidity pool
  uint256 public immutable initialMint;

  /// @notice Ratio of tokens minted to be given to deployer
  /// The use of uint8 (0...255 range) allows for a min resolution of about 0.4%
  /// i.e. (2 == ~0.78%) - (127 == ~49.80%) - (254 == ~99.60%)
  uint8 public immutable _toDeployer;

  /// @notice Actual amount of tokens decimals transfered to deployer during {initialize}
  uint256 public toDeployer;

  /// @notice Liquidity pool token
  /// @dev optional, see NOTE in {initialize}
  IUniswapV2Pair public pair;

  /// @notice True if the contract was properly initialized and the LP seeded, meaning no more tokens can be minted. See also {initialize}
  ...
  function initialize(address routerAddress_) public payable {
    ...
    (uint amountTokenFinal, uint amountWETHFinal, uint liquidity) = router_.addLiquidityETH{value: msg.value}(
